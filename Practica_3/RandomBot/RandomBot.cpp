...
Move RandomBot::nextMove(const vector<Move> &adversary, const GameState &state) {
	...
	for (int i= 1; i<=6;i++) {
		if (state.getSeedsAt(turno, (Position) i) >0)
			available++;	// Cuenta el número de movimientos disponibles
	}

	int n= 1+ (rand()%available);	// número aleatorio entre el número de movimientos disponibles
	int aux= 0;

	for (int i= 1; i<=6 && movimiento == M_NONE; i++) {
		if (state.getSeedsAt(turno, (Position) i) >0)
			aux++;	// Si hay semillas aumento aux
		if (aux == n)	// Si aux==n elijo dicho movimiento
			movimiento= (Move)i;
	}
